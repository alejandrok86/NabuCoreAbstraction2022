...
{
    [Route("Response/ResponseOutcome")]
    [ApiController]
    [ApiExplorerSettings(GroupName = "Response")]
    public class ResponseOutcomeController : ControllerBase
    {
        private IConfiguration _config;
        private BaseVersion release = new BaseVersion("ResponseOutcome API", 1, 0, 0, "");

        public ResponseOutcomeController(IConfiguration config)
        {
            _config = config;
        }
         ...
         * List
         ***********************************************************************************************************************************/
        [HttpGet("List/{pResponseID}")]
        public IActionResult List(int pResponseID)
        {
            List<Entities.Response.ResponseOutcome> responseOutcomes = new List<Entities.Response.ResponseOutcome>();
            if (Request.Headers.ContainsKey("APIKey") && Request.Headers.ContainsKey("APILicensedTo"))
            {
                MethodBase method = MethodBase.GetCurrentMethod();
                APIAccessKey apiAccess = new APIAccessKey();
                ...
                if (state == APIKeyState.KeyValidAccessGranted)
                {
                    ...
                    return Ok(responseAbstraction.ListResponseOutcomes(pResponseID));
                }
                else
                {
                    apiAccess.AuditAccess(_config.GetValue<string>("APIKeyConfig:AuditFolder"), release.Component, method.Name, state.ToString(), Helper.APICallerInfo.GetIPAddress(HttpContext), Helper.APICallerInfo.GetUserAgent(Request));
                    ...
                    responseOutcomes.Add(error);
                    return Unauthorized(responseOutcomes.ToArray());
                }
            }
            else
            {
